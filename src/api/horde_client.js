...
export class HordeClient {
  ...
  async getCompletion(story, callback) {
    let client = this
    this.story = story
    if (this.story.messages == undefined){
      this.story.messages = []
    }
    if (this.story.options == undefined){
      this.story.options = {}
      this.story.options.heros = {}
    }
  ...
  //story.data.operation.data.prompt
  let locale = story.options.lang || 'fr'
  this.lang = this.langues[locale] || 'french'
  this.prenom = story.options.heros.prenom || "Isidore"
  this.sexe = story.options.heros.sexe || "garçon"
  console.log(this.lang, this.prenom, this.sexe)
  this.story.messages.push({isUser: true, text: story.data.operation.data.prompt})


  let system_prompt_test= [
    "L'invite ci-dessous est une question à laquelle répondre, une tâche à accomplir ou une conversation à laquelle répondre. Décide et rédige une réponse appropriée. Décris les étapes pas à pas.",
    "[INST]{{.Input}}[/INST]"
]
  let system_prompt_brut = /*story.options.mission[1].*/system_prompt_test.join('\n')
  ...
  generateCompletionPromptNumerai(story) {
