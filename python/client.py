...
class YDocWSClient:
    ...
    def __init__(self, doc:Y.YDoc, uri = "wss://ypu.glitch.me/my-ypu"):
        self.send_q = queue.Queue()
        self.recv_q = queue.Queue()
        self.uri = uri
        self.doc = doc
        self.todos = self.doc.get_map("todos")
        def async_loop():
    ...
    def send_updates(self, txn_event: Y.AfterTransactionEvent):
       ...
       # print("send_updates",update)
        if update != b'\x00\x00':
            print("send",update)
            self.send_q.put_nowait(update)

        # todos = self.doc.get_map("todos")
        # pending = self.doc.get_map("pending")
        # done = self.doc.get_map("done")
        # print("todos",todos)
